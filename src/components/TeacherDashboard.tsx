...
import { GraduationCap, Users, AlertCircle, TrendingUp, Loader2, Lock, User as UserIcon, ArrowRight, Download, Plus, Trash2, X, Search, FileText, Save, ChevronLeft, Edit2, Sparkles, ClipboardList, Calendar } from '../icons';
...
const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ user, isAuthenticated, setIsAuthenticated, students, setStudents }) => {
  ...
  const handleGenerateAssignment = async () => {
    ...
    const assignment = await generateAssignment(assignmentTopic);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8 animate-fade-in relative pb-24">
      ...
      {/* AI Assignment Generator Section */}
      <div className="bg-white p-6 rounded-xl border shadow-sm mt-8">
        <h3 className="font-bold text-lg text-slate-800 mb-4 flex items-center gap-2">
           <Sparkles className="text-amber-500" /> AI Assignment Generator
        </h3>
        <p className="text-slate-600 mb-6">Create high-quality, creative assignments for your students in seconds using Gemini AI.</p>
        
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="flex-1 relative">
            <ClipboardList className="absolute left-3 top-3 text-slate-400" size={20} />
            <input 
              type="text" 
              placeholder="Enter assignment topic (e.g., Quantum Mechanics Lab, English Poetry Analysis)..."
              className="w-full pl-11 pr-4 py-3 border rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-50 focus:bg-white transition-all"
              value={assignmentTopic}
              onChange={(e) => setAssignmentTopic(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && handleGenerateAssignment()}
            />
          </div>
          <button 
            onClick={handleGenerateAssignment}
            disabled={generatingAssignment || !assignmentTopic}
            className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 disabled:opacity-50 transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-100 group"
          >
            {generatingAssignment ? <Loader2 className="animate-spin" size={20} /> : <Sparkles size={20} className="group-hover:animate-bounce" />}
            {generatingAssignment ? 'Generating...' : 'Generate with AI'}
          </button>
        </div>

        {generatedAssignment && (
          <div className="bg-slate-50 border rounded-2xl p-6 animate-fade-in">
            <div className="flex justify-between items-start mb-4">
              <h4 className="text-xl font-bold text-slate-900">{generatedAssignment.title}</h4>
              <div className="flex items-center gap-2 text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                <Calendar size={14} /> {generatedAssignment.deadline}
              </div>
            </div>
            
            <div className="space-y-4">
              <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">Learning Tasks:</p>
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {generatedAssignment.tasks.map((task, i) => (
                  <li key={i} className="bg-white p-4 rounded-xl border flex gap-3 items-start group hover:border-indigo-200 transition-colors">
                    <div className="w-6 h-6 bg-indigo-50 rounded-full flex items-center justify-center text-indigo-600 text-xs font-bold shrink-0">
                      {i + 1}
                    </div>
            ...
            <div className="mt-8 flex gap-3">
              <button 
                onClick={() => alert("Assignment published to all student portals!")}
                className="bg-slate-900 text-white px-6 py-2.5 rounded-lg text-sm font-bold hover:bg-slate-800 transition-all flex items-center gap-2"
              >
                Publish to Students
              </button>
              <button 
                onClick={() => setGeneratedAssignment(null)}
                className="text-slate-500 px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-slate-100 transition-all"
              >
                Discard
              </button>
            </div>
